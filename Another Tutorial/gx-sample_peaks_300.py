...
df = pd.read_csv("C:\\Users\\TechTeam-08\\PycharmProjects\\Essential Data Quality Principles, Implement Testing with Python and Great Expectations Framework\\sample_peaks_300.csv")
...
expectation_results["accuracy"].append(
    validator.expect_column_values_to_be_between("height", min_value=1111, max_value=9999)
)

# Validity Expectations
expectation_results["validity"].append(
    validator.expect_column_values_to_be_of_type("height", type_="int64")
)
expectation_results["validity"].append(
    validator.expect_column_values_to_be_of_type("id", type_="int64")
)
expectation_results["validity"].append(
    validator.expect_column_values_to_match_regex("name", regex="^[A-Za-z\\s]+$")
)
